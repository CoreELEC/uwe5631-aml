...
static void sdiohal_throughput_tx_compute_time(void)
{
	...
	tp_tx_cnt++;
	if (tp_tx_cnt % 500 == 0) {
		wcn_getnstimeofday(&tp_tx_stop_time);
		...
		tp_tx_cnt = 0;
		wcn_getnstimeofday(&tp_tx_start_time);
	}
		   ...
		   struct mbuf_t *tail, int num)
{
	...
	if (tp_rx_cnt / (500*64) == 1) {
		wcn_getnstimeofday(&tp_rx_stop_time);
		...
		tp_rx_cnt = 0;
		wcn_getnstimeofday(&tp_rx_start_time);
	}
	wcn_getnstimeofday(&tp_tm_begin);
