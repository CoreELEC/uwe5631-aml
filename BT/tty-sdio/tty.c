...
/*
 * of_property_read_string()/struct of_device_id need <linux/of.h> and
 * <linux/of_device.h>/<linux/mod_devicetable.h>. These used to be
 * guarded behind CONFIG_OF (relevant on very old kernels lacking
 * devicetree headers entirely), but at our 5.15+ floor these headers
 * always exist -- they provide harmless static-inline stubs when
 * CONFIG_OF=n -- so the include is unconditional rather than gated,
 * to avoid "undeclared function"/"incomplete type" errors on builds
 * where CONFIG_OF isn't set for this compilation.
 */
#include <linux/of.h>
...
/*
 * struct platform_driver::remove() returns void, not int, as of Linux
 * 6.11 (commit 0edb555a65d1, "platform: make platform_driver::remove()
 * return void"). mtty_remove() only ever returns 0, so the conversion
 * is a pure signature change.
 */
...
/*
 * struct tty_operations::write/write_room changed signature/return
 * type in Linux 6.6.1 (part of a wider tty-core cleanup converting
 * `unsigned char`/`int` to `u8`/`size_t`/`ssize_t`/`unsigned int`
 * throughout the tty layer): .write went from
 * `int (*)(struct tty_struct *, const unsigned char *, int)` to
...
static int mtty_tty_driver_init(struct mtty_device *device)
{
    ...
    /*
     * alloc_tty_driver() was removed in Linux 5.14 (our floor is
     * 5.15); tty_alloc_driver() is the replacement -- it takes the
     * flags directly (TTY_DRIVER_REAL_RAW used to be set separately
     * below via driver->flags) and returns an ERR_PTR() on failure
     * instead of NULL.
     */
    ...
    */
    ...
    if (ret)
    {
        /* put_tty_driver() was removed in Linux 5.15 (our floor). */
...
/*
 * sprdbt_tty calls symbols EXPORT_SYMBOL'd by uwe5621_bsp_sdio
 * (sprdwcn_bus_*). The BUILD.bazel `deps` wiring already makes
